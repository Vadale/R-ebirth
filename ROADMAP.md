...
### Phase 11 — Multimodal models ✅ SHIPPED 2026-07-15 (`v0.2.0`) *(pulled forward per D-023 — unlocks radiology images)*

> **Exit met.** WP-V1 (re-vendor + `libmtmd`, PR #32), WP-V2 (image FFI + T1, PR #33), WP-V3 (T2 `llm_embed(images=)`, PR #34), WP-V4 (goldens/nightly/registry/docs, PR #35), and the post-nightly correctness round (PR #37, D-026 fourth addendum) are merged. Verified at the tag: the r-universe binary, installed into a clean library on the founder's M4, answered a question about an image on Metal. Delivered as **T1+T2 only** — T3 (interpretability of the vision tower itself) remains a separate later research phase, unstarted. **Audio is not shipped**, though `libmtmd` carries it (D-026 Option A).
