...
Returns a **new `llm` handle** with a steering intervention added (adds `coef * direction` to the residual stream at `layer` for the given positions during any subsequent forward pass). `direction` = numeric vector of length `hidden_size` (checked). The original handle is untouched — removal = use the original object. Interventions compose: steering a steered handle stacks both. Errors: `rebirth_error_intervention` (dimension mismatch, invalid layer).

### `llm_ablate(m, layer, neurons, value = 0, component = "residual")`
Same pattern: new handle with the listed 1-based `neurons` of `component` at `layer` forced to `value` during forward passes. Errors: `rebirth_error_intervention`.
