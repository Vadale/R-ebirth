...
6. **Plan the test order:** write the failing tests first where practical (testthat/cargo). For numerical work, plan the goldens now (see the `golden-update` skill) — golden-first rule.
7. **Implement** per the `coder` agent rules (small steps, verify each, honest reporting).
8. **Before handing over:** `devtools::test()`, `R CMD check`, `cargo test && cargo clippy && cargo fmt --check` all green locally; `NEWS.md` updated if user-visible; then run the `reviewer` agent, then the founder reviews.
