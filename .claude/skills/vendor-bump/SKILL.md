...
7. **Full verification, in order:** `verify_vendored_tree.sh` (G4 + patch coherence, step 5) → `cargo test` → harness B on the synthetic model (must be exact; the un-intervened goldens must stay byte-identical after re-applying the patch) → harness B nightly suite on the CI model → the two demos on the founder's Mac. Any golden movement goes through the `golden-update` skill with reason = this bump.
   - **Multimodal pins to re-validate** (WP-V1..V3, D-026): the `mtmd_context_params` ABI test values; the mmproj-mismatch format string parsed by `parse_embd_mismatch` (vision.rs pins the mtmd.cpp literal); that `is_audio_file` is still the ONLY miniaudio gateway (audit req 9); and that `MTMD_INPUT_CHUNK_TYPE_TEXT` is still the enum's first value, `0` (ffi.rs pins it; the T2 embed loop branches on it).
