...
5. **Re-apply the tap patch set** from `vendor/patches/`. If a hunk fails, port it by hand *minimally* — the patch stays as small as upstream allows. Update the patch files to match.
...
7. **Full verification, in order:** `cargo test` → harness B on the synthetic model (must be exact) → harness B nightly suite on the CI model → the two demos on the founder's Mac. Any golden movement goes through the `golden-update` skill with reason = this bump.
