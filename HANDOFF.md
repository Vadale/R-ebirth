...
You are taking over development of **R-ebirth**: an R package (`rebirth`) with a Rust
...
- Errors reach R as **classed conditions** (`rebirth_error_*`), never raw Rust
...
- **WP2** — `llm_tokens()` (UTF-8 encode/decode) and `llm_generate()`: greedy
  ...
  `rebirth_error_context_overflow`. A simplifier pass followed.
...
- **WP3 — `llm_embed(m, x, pooling = c("mean","last","model"), normalize = TRUE)`**
  ...
  `seq_along(x)`. Errors: `rebirth_error_embed`. **This is the last Phase-1 WP.**
...
- **Scope discipline (D-010, 2026-07-06):** `v1.0` stays the *lean* core
  ...
  (Phase 12), alignment/RL (Phase 13), topics+SAE (Phase 14) and `rebirth.bio`
...
- `rebirth/` — the R package.
  ...
  - `src/rust/` — Cargo workspace: `rebirth-ffi/` (the extendr boundary, `[lib]
    name = "rebirth"`, the *only* crate that speaks R) and `rebirth-llm/` (the
...
- Test model: `~/models/qwen2.5-0.5b-instruct-q8_0.gguf`; gate real-model
  ("[MODEL]") tests on env `REBIRTH_TEST_MODEL_QWEN`, which skip in CI/CRAN. The
  build needs `cmake`.
