...
**Capture spec → memory estimate:** `bytes ≈ n_prompts × n_positions × n_layers × n_components × hidden_size × 4` (f32 host buffers) + arrow overhead. Computed *before* running; drives the predictive OOM check and the spill decision (§6). The estimate and the filter suggestion appear verbatim in `rebirth_error_oom`.
...
- **Budget:** default in-memory threshold = `min(2 GB, 20% of system RAM)`, overridable via `options(rebirth.trace_budget = <bytes>)`. Above it, `spill = TRUE` streams to disk; `spill = FALSE` raises the predictive `rebirth_error_oom`.
