...
# checks are [MODEL]-gated on RELM_TEST_MODEL_QWEN (Step 6 / local hardware).
...
test_that("llm_embed() rejects a non-llm handle", {
  expect_error(llm_embed(42, "hi"), class = "relm_error_argument")
})
...
test_that("llm_embed() validates its arguments", {
  ...
  expect_error(llm_embed(m, character(0)), class = "relm_error_argument")
  expect_error(llm_embed(m, NA_character_), class = "relm_error_argument")
  expect_error(llm_embed(m, 42), class = "relm_error_argument")
  expect_error(llm_embed(m, c("a", NA)), class = "relm_error_argument")
  expect_error(llm_embed(m, c("a", "")), class = "relm_error_argument")
  expect_error(llm_embed(m, "hi", normalize = NA), class = "relm_error_argument")
  expect_error(llm_embed(m, "hi", normalize = "yes"), class = "relm_error_argument")
  expect_error(
...
test_that("llm_embed() rejects a closed handle", {
  m <- llm(synthetic_model_path())
  close(m)
  expect_error(llm_embed(m, "hi"), class = "relm_error_closed")
})
...
test_that("pooling = \"model\" on a generative model is a clean embed error", {
  ...
  expect_error(llm_embed(m, "hello", pooling = "model"), class = "relm_error_embed")
})
