...
#   2. The no_vocab synthetic model raises rebirth_error_tokenize, never a crash
#      (download-free, runs in CI).
#   3. Real-model round-trips incl. Italian accented text (env-gated on
#      REBIRTH_TEST_MODEL_QWEN, skipped in CI).

qwen_model <- function() {
  path <- Sys.getenv("REBIRTH_TEST_MODEL_QWEN")
  skip_if(!nzchar(path), "REBIRTH_TEST_MODEL_QWEN not set")
  skip_if_not(file.exists(path), "REBIRTH_TEST_MODEL_QWEN points at a missing file")
  path
}
...
test_that("llm_tokens rejects a non-logical decode", {
  m <- stub_llm()
  expect_error(llm_tokens(m, "hi", decode = "yes"), class = "rebirth_error_argument")
  expect_error(llm_tokens(m, "hi", decode = c(TRUE, FALSE)), class = "rebirth_error_argument")
  expect_error(llm_tokens(m, "hi", decode = NA), class = "rebirth_error_argument")
})

test_that("llm_tokens (encode) rejects non-character x", {
  m <- stub_llm()
  expect_error(llm_tokens(m, 1:3), class = "rebirth_error_tokenize")
  expect_error(llm_tokens(m, c("a", NA)), class = "rebirth_error_tokenize")
})

test_that("llm_tokens (decode) rejects non-integer or non-positive ids", {
  m <- stub_llm()
  expect_error(llm_tokens(m, c(1.5, 2), decode = TRUE), class = "rebirth_error_tokenize")
  expect_error(llm_tokens(m, "abc", decode = TRUE), class = "rebirth_error_tokenize")
  # 0 is invalid: token ids are 1-based in the R API.
  expect_error(llm_tokens(m, c(1L, 0L), decode = TRUE), class = "rebirth_error_tokenize")
})

test_that("llm_tokens on a closed handle raises rebirth_error_closed", {
  m <- empty_handle_llm()
  close(m)
  expect_error(llm_tokens(m, "hi"), class = "rebirth_error_closed")
})

# --- 2. no_vocab model: a classed condition, not a crash -------------------

test_that("llm_tokens on a no_vocab model raises rebirth_error_tokenize", {
  m <- llm(synthetic_model_path(), backend = "cpu")
  on.exit(close(m), add = TRUE)
  expect_error(llm_tokens(m, "hello"), class = "rebirth_error_tokenize")
  expect_error(llm_tokens(m, c(1L, 2L), decode = TRUE), class = "rebirth_error_tokenize")
})
...
test_that("decode rejects an id outside the vocabulary (classed, not a crash)", {
  ...
  expect_error(llm_tokens(m, huge, decode = TRUE), class = "rebirth_error_tokenize")
})
