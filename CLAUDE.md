...
**Status (2026-07-06):** **code underway — Phase 1 complete (3/3); Phase 2 (the anatomy lab) underway — WP4 merged (PR #7).** WP0 (bootstrap + CI), WP1 (`llm()` model loading over vendored llama.cpp b9726, Metal), WP6a (Harness B synthetic oracle), WP2 (`llm_tokens()`/`llm_generate()`, token-for-token validated), WP3 (`llm_embed()`, synthetic-golden + real-Qwen validated, ADR D-011), and **WP4 (`llm_trace()` — activation taps + disk spill)** are merged to `main`; CI green cross-platform. WP4 delivered zero-patch eval-callback observation (D-012) on a dedicated transient trace context, the `attn_out` post-projection semantics (D-014), and Arrow-IPC disk spill (D-013: `nanoarrow` + `arrow-ipc`, feature-gated) with the project's first background thread; the synthetic activation golden matches the numpy oracle to max |Δ| 3.7e-3; it passed the security-auditor (memory-safe), reviewer, and simplifier gate. **Next in Phase 2: WP5 (`llm_steer`/`llm_ablate` — the intervention core, incl. the D-012-pre-authorized `build_cvec` ablation patch) then `llm_logits`.** `API-GRAMMAR.md` v1.0 binding (D-003); `DECISIONS.md` at D-014 (+ rung-3 fork playbook, Appendix A). See `HANDOFF.md` for the full development handoff. Exported so far: `llm()`, `llm_tokens()`, `llm_generate()`, `llm_embed()`, `llm_trace()` + S3 methods. **WP4 follow-ups (deferred, tracked):** HF-Qwen activation-golden generation script → WP6b (nightly tolerance + rank-corr + mutation test); gate the `rebirth_selftest_*` FFI behind a non-default `selftest` cargo feature; wire `cargo audit`/`cargo deny` in CI (D-008 G4); the `[MODEL]` 4B-spill + Qwen-tolerance acceptances run on the founder's Mac (Metal validation of the tap). Remaining founder input still open: HF account with MedGemma terms (thesis-era, parked); repo visibility; optional non-blocking: pin a small embedding GGUF (BERT/nomic/bge-class) for automated non-causal embedding coverage (`docs/wp3-embed-plan.md` §9).
