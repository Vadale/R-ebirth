...
## Status (2026-07-15) — `v0.2.0` SHIPPED

**`relm` v0.2.0 (vision) is public**, tagged at `b8bd7de` with a GitHub release. Install:
`install.packages("relm", repos = c("https://vadale.r-universe.dev", getOption("repos")))`
Binaries on r-universe: **macOS arm64 + macOS Intel + Linux** (aarch64 + x86_64), R 4.5.3/4.6.1/4.7.0. Windows = Phase 8. Verified at the tag by installing the r-universe binary into a clean library on the founder's M4 and running vision end-to-end (Metal). v0.1.0 (text-only) shipped the same way; the repo is public and the Show HN is posted.

**Phases complete: 1, 2 (anatomy lab), 3 (v0.1.0), 11 (vision).** Next: **Phase 8 = Windows** (native + CUDA on the founder's RTX 2060; WSL2 first).

**Exported API:** `llm()`, `llm_tokens()`, `llm_generate()`, `llm_embed()`, `llm_trace()`, `llm_steer()`, `llm_ablate()`, `llm_logits()`, `llm_download()` + S3 methods. Vision rides the existing grammar: `llm(projector=)`, `llm_generate(images=)`, `llm_embed(images=)`. `API-GRAMMAR.md` v1.0 is binding (D-003); `DECISIONS.md` is at **D-026** — read it for anything below, it is the reasoned record.

**How the engine is built and checked** (the load-bearing facts; details in `DECISIONS.md` + `ARCHITECTURE.md`):
- Vendored llama.cpp **b9726**, patched. `0001` is the `build_cvec` ablation hook (llama-side only); `0002` touches two `CMakeLists.txt`. **No relm patch touches clip/mtmd/ggml.** D-015 governs patch application (committed patched tree + G4 post-patch-SHA gate + reverse-apply coherence check, both in CI).
- Observation is zero-patch via eval-callback (D-012); `attn_out` is post-projection (D-014); traces spill to Arrow IPC (D-013); the trace budget is measured on **materialized R bytes**, not an engine representation (D-017).
- Intervention semantics: ablate-after-steer, steer-sums / ablate-union, arch allow-list `{llama,qwen2,gemma3}`; handles are fresh contexts on a cloned `Arc<Model>` → bit-for-bit reversible (D-016).
- **Harness B is dual-reference (D-018):** same-implementation legs are exact gates (synthetic numpy oracle; unpatched-llama.cpp logits); the HF-fp32 leg is a scale-robust cross-check (Spearman/cosine ≈ 0.94) anchored by the exact residual-decomposition identity. ≥ 0.999/layer against an independent HF reference **is not achievable** — that divergence is intrinsic to llama.cpp-vs-PyTorch, not quantization or backend.
- **A float reference belongs to the MACHINE that produced it, not its OS/arch** (D-026 fourth addendum — the lesson of the first real nightly). The vision nightly rebuilds pristine b9726 on its own runner and compares against that, so the encoder leg is exact everywhere (`max |Δ| = 0.0`) with no tolerance to tune. **Scope:** that leg gates relm's libmtmd API usage only — the encoder path carries none of relm's patch, so `0.0` there is the expected result and is blind to `build_cvec`. What carries vision cross-ISA is the **byte-exact T1 text golden + token-ids pin**, green on Linux against an arm-recorded reference.

**Known, and now diagnosed (D-026 fifth addendum):** upstream's image encoder is float-path-dependent. Building pristine b9726 **three times on one runner**, changing only the vector path, moves **every one of 98,304 values** (`max |Δ| = 1.624` between AVX2+FMA and baseline SSE2; `native == avx2` bit-for-bit as the control) — the same order and signature as the arm-vs-x86 gap (`3.300`), which is larger because it swaps whole kernels. It also explains why two `ubuntu-24.04` runners disagreed (`3.30` vs `8.71`): the pristine build never passes `-DGGML_NATIVE=OFF`, so each runner bakes in its own ISA and GitHub's pool is heterogeneous. **Benign at the level that ships** (cosine `0.9998`; the byte-exact T1 text golden passes cross-ISA — identical semantics), **not** benign as "differs in the last decimals" — individual values move by tens of percent. It is upstream's arithmetic, not ours; nothing was changed and no tolerance was invented.

**Deferred (tracked):**
- **F-1 partly delivered (D-019):** nightly Valgrind memory-safety + leak job (intervention path, download-free) and the `cargo audit`/`cargo deny` supply-chain gate are wired. **Still open:** the deeper ASan/UBSan job with the vendored llama.cpp recompiled `-fsanitize=address,undefined` (catches the stack/global overflows + non-memory UB Valgrind misses); gate `rebirth_selftest_*` behind a non-default `selftest` cargo feature.
- The nightly reference's provenance is bound by filename, not a content digest (rule-8d shape; the T2 pin's operator-asserted key was the same defect and is now fixed — D-026 fifth addendum — so this is the last one of its family).
- `[MODEL]` acceptances that need the founder's Mac: 4B-spill, Qwen tolerance, WP5 valence/KL.
- **Founder input open:** HF account with MedGemma terms (thesis-era, parked). Non-blocking: pin a small embedding GGUF for automated non-causal embedding coverage (`docs/wp3-embed-plan.md` §9).
