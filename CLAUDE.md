...
**Status (2026-07-06):** **code underway — Phase 1 complete (3/3); Phase 2 (the anatomy lab) started.** WP0 (bootstrap + CI), WP1 (`llm()` model loading over vendored llama.cpp b9726, Metal), WP6a (Harness B synthetic oracle), WP2 (`llm_tokens()`/`llm_generate()`, token-for-token validated), and WP3 (`llm_embed()`, synthetic-golden + real-Qwen validated, ADR D-011) are merged to `main`; CI green cross-platform. **Now in WP4 (`llm_trace()` — activation taps):** the mandatory day-1 spike ran against b9726 and its two ADRs are accepted (D-012 tap strategy = zero-patch eval-callback observation, with a guarded `build_cvec` patch pre-authorized for WP5 ablation; D-013 spill deps = `nanoarrow` + `arrow-ipc`, feature-gated), both independently concurred by a Fable-5 review; implementation proceeds golden-first from Step 2 (`docs/wp4-trace-plan.md`). Phase 2 remainder = `llm_steer`/`llm_ablate`/`llm_logits`. `API-GRAMMAR.md` v1.0 binding (D-003); `DECISIONS.md` at D-013 (+ rung-3 fork playbook, Appendix A). See `HANDOFF.md` for the full development handoff. Exported so far: `llm()`, `llm_tokens()`, `llm_generate()`, `llm_embed()` + S3 methods. Remaining founder input still open: HF account with MedGemma terms (thesis-era, parked); repo visibility; optional non-blocking: pin a small embedding GGUF (BERT/nomic/bge-class) for automated non-causal embedding coverage (`docs/wp3-embed-plan.md` §9).
