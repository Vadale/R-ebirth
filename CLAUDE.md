...
**Known and unresolved:** upstream's own image encoder varies across ISAs (`max |Δ| = 3.30` isolated; cross-machine values reach `8.71`, attributed to the ISA by inference, not isolation). Real, undiagnosed, shipped to Linux users, and **structurally invisible to a same-machine comparison by design**. Leading hypothesis (untested, ~5 min to check from ggml's CPU-feature log): x86 SIMD width changes the reduction order and a ViT amplifies it.
...
- **The T2 vision pin runs nowhere unless someone remembers `RELM_VISION_RECORDING_MACHINE=1`** — an operator-asserted key, which is the shape hard rule 8d forbids. Bind it to a machine fingerprint. (The nightly's T2 coverage meanwhile is the cat-vs-car semantic gate.)
- The nightly reference's provenance is bound by filename, not a content digest (same rule-8d shape).
