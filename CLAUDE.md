...
**Status (2026-07-06):** **code underway — roadmap Phase 1 is complete (3/3).** WP0 (bootstrap + CI), WP1 (`llm()` model loading over vendored llama.cpp b9726, Metal), WP6a (Harness B synthetic oracle), WP2 (`llm_tokens()`/`llm_generate()`, token-for-token validated), and WP3 (`llm_embed()`, synthetic-golden + real-Qwen validated, ADR D-011) are merged to `main`; CI green cross-platform. **Phase 1 is closed; next is Phase 2** = the anatomy lab (`llm_trace`/`llm_steer`/`llm_ablate`/`llm_logits`), the interpretability core. `API-GRAMMAR.md` v1.0 binding (D-003); `DECISIONS.md` at D-011 (+ rung-3 fork playbook, Appendix A). See `HANDOFF.md` for the full development handoff. Exported so far: `llm()`, `llm_tokens()`, `llm_generate()`, `llm_embed()` + S3 methods. Remaining founder input still open: HF account with MedGemma terms (thesis-era, parked); repo visibility; optional non-blocking: pin a small embedding GGUF (BERT/nomic/bge-class) for automated non-causal embedding coverage (`docs/wp3-embed-plan.md` §9).
