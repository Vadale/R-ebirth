...
| Untrusted-input asserts: `clip_image_size::area()` `GGML_ASSERT(width/height ≤ 46000)` → `abort()` on oversized dims (uncatchable, like D-008 gate G1). `mtmd.cpp` catches C++ exceptions internally (14 `try/catch`), so exceptions do not cross the `extern "C"` boundary. | b9726:tools/mtmd/clip.h L28–29; mtmd.cpp (try/catch count) |
...
| 2 | **Image parsing = untrusted input** (stb_image CVE history; clip's `GGML_ASSERT(dim ≤ 46000)` → uncatchable `abort()`, the D-008 G1 class). | M×H | **Mandatory security-auditor gate** (ROADMAP): magic-byte allow-list before decode (§3.3); validate image dimensions/pixel-count in Rust **before** the FFI (reject-not-clamp, hard rule 8b → `relm_error_image`) so clip's assert is never reached; decode on the R main thread (no new threads → D-008 G2 preserved); confirm mtmd's C API catches internally (14 try/catch verified). Auditor may require Option B (§3.3). |
...
**In scope.** The FFI symbols the T1 flow needs (`mtmd_init_from_file`, `mtmd_context_params_default`, `mtmd_free`, `mtmd_bitmap_init`/`mtmd_bitmap_free`, `mtmd_tokenize`, `mtmd_input_chunks_*`, `mtmd_helper_eval_chunks`, `mtmd_support_vision`, `mtmd_default_marker`, `clip_n_mmproj_embd` for the mismatch check); the interleaved ingest via `mtmd_helper_eval_chunks(n_batch)` then the existing sampler loop; mmproj/model-arch mismatch → `relm_error_image` (reject-not-clamp); the `images` pairing/recycling semantics (§7); a committed test image (small, license-clean, e.g. a solid-color or simple synthetic PNG for the deterministic gate + a real photo for `[MAC]`).
...
- `[CI]` model-free unit tests: the magic-byte allow-list accepts JPEG/PNG/BMP/GIF and rejects audio/garbage with `relm_error_image`; an oversized-dimension input is rejected **before** the FFI; a mismatched mmproj/model pair → `relm_error_image` naming both embd sizes; `images` on a handle loaded without a projector → `relm_error_image`.
